...
class Counseling extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['intervention_id','type','session'], 'required'],
            ['session', 'unique'],
            ['other_clients','safe'],
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if($insert)
        {
           if(($this->other_clients))
           {
               $clients = explode(",", $this->other_clients);
               foreach($clients as $client)
               {
                   // Insert an intervention record
                   $model = new Intervention();
                   $model->intervention_type_id = 2; //counselling
                   $model->client_id = $client;
                   $model->situation_description = 'Group Counselling Code: '. $this->code.' & Session No: '.$this->session;
                   $model->case_id = 1; // default to insecurity

                   if(!$model->save())
                   {
                       print_rr($model->errors);
                       exit;
                   }
